...
#Create a blueprint and set the url prefix for routes in this file
...
#  dashboard route, redirects to dashboard page and initiates the dashboard page
...
# profile route, redirects to profile page and gets user and order info
@views.route('/profile')
@cross_origin()
def profile():
    try:
...
# order route, handles making a new order or getting an existing one
@views.route('/order')
@cross_origin()
def order():
    try:
...
# help route, initiates route page
@views.route("/help")
@cross_origin()
def help_page():
    try:
...
# help editor, initiates editor page
@views.route("/editor")
@cross_origin()
def editor():
    try:
